...
function requireEnvironment(name: string, pattern: RegExp): string {
  ...
  if (value === undefined || !pattern.test(value)) {
    throw new Error(
      `${name} is missing or invalid; use scripts/run-x1-transport-evidence.ts.`,
    );
  }
  return value;
}
...
const expectedHarnessPath = resolve(
  runDirectory,
  "harness/x1-transport-browser-harness.js",
);
...
export const X1_TRANSPORT_TEST_TIMEOUT_MS = 300_000;
export const X1_TRANSPORT_GLOBAL_TIMEOUT_MS = 1_200_000;

export default defineConfig({
  testDir: resolve(root, "tests/integration"),
  testMatch: "transport-browser-evidence.test.ts",
  ...
  retries: 0,
  workers: 1,
  timeout: X1_TRANSPORT_TEST_TIMEOUT_MS,
  globalTimeout: X1_TRANSPORT_GLOBAL_TIMEOUT_MS,
  expect: {
    timeout: 10_000,
  },
  reporter: [
