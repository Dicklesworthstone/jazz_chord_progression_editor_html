...
### Print font metrics — 2026-09-16 continuation
...
- [x] Correct the earlier visual diagnosis: retained sequential/single-page/normalized-FontBBox Letter page-two PNGs are byte-identical (SHA-256 `2414723f4132c8c48b5f33eb600bbc738f74068d982c7a729c18849da8227e60`). All four original native PDFs reproduce identical sequential/individual pixels. No missing-glyph defect or FontBBox repair is established; the prior visual claim is retracted.
- [x] Complete a two-case real Chromium variable/static-font experiment through RCH (exit 0); static weight 400 embeds as CID TrueType. Original probe wrapper exit 1 came from omitting its spec filename and triggering an unrelated evidence merge; initial red evidence remains retained.
- [x] Independently verify the actual defect: layout advances were taken at the variable font's default weight, while preview/download request weight 400. Pin FreeType-derived metrics and browser fit evidence before changing production.
- [x] Use the existing bundled font at static weight 400 for exact print metrics, with reproducible checked-in derivation, unchanged repertoire and source/layout limits. Retain the ordinary UI's variable font.
- [x] Isolate viewport-dependent native SVG width rounding in a real browser counterfactual; request `geometricPrecision` in both preview and standalone SVG, retaining the original .1 mm independent-width assertion.
- [x] Pass RCH final code gates: 13 focused tests / 301 assertions; full types/lint/boundaries; reproducible font/CSS; guarded build; E2E types/lint and 27 Chromium/Firefox/WebKit cases, zero unexpected/skipped/flaky. Inspect 27 clean error/request records, 36 SVGs and four native PDF proofs; all 1,804 frozen inputs remain unchanged.
- [x] Rebuild immediately before commit and commit/push `9e6f4dc`. RCH committed-byte gates pass: 11 shipping models, nine instrument-quality profiles with zero failing findings, all 15 playback instruments and enforced recovery; committed rebuild is identical. All 1,804 inputs and mtimes remain current.
- [x] Inspect native output and deploy committed `9e6f4dc` to Cloudflare `d1aafd9a` and Vercel `dpl_5pWQ3M57TR1ttqNbwqcgSQjJV4Ns`. Both hosts serve the committed 8,636,009-byte HTML, SHA-256 `22a32f94a005c62ce0513df7a41f2aa25161f2258f36ea31e168a1835f611324`, and identical committed share-image bytes.
- [x] Pass 18 live Chromium cases (nine per host): A4/Letter at 320/1280px, exact downloaded SVGs and native PDFs, source invalidation, fitting/overfull title pair; zero unexpected/skipped/flaky or application errors. Inspect desktop/phone-width screenshots, 24 SVGs, eight PDFs and 18 error/request records. Cloudflare contributes one expected blocked analytics-script CSP diagnostic per case; Vercel contributes none. CSP remains unchanged.
- [x] Preserve interrupted/failed verification attempts separately: initial live wrapper termination produced no complete browser JSON; first detached restart was launched before its helper upload completed and ran no tests. Correct upload ordering plus a detached RCH wrapper produced the complete green run in `test-results/print-metrics-live-complete-20260916/`. Earlier code/probe red results remain retained.
- [x] Record the HTTP-client distinction: Python responses also receive Cloudflare analytics injection here; raw curl responses on both hosts match committed bytes. Never treat that raw hash as proof that browser injection is absent.
- [ ] Complete physical A4/Letter printer output, actual native-phone printing and independent acceptance on original `.11.3`; return it to open rather than claiming those gates from viewport automation.
