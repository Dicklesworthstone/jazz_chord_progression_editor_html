...
/**
 * U4/verify (jcpe-milestone-reliable-studio-l3a.12.3) pinned Playwright
 * configuration: the U4 transport-controls evidence spec against the
...
function requireEnvironment(name: string, pattern: RegExp): string {
  ...
  if (value === undefined || !pattern.test(value)) {
    throw new Error(
      `${name} is missing or invalid; use scripts/run-u4-transport-evidence.ts.`,
    );
  }
  return value;
}
...
export const U4_TRANSPORT_TEST_TIMEOUT_MS = 300_000;
export const U4_TRANSPORT_GLOBAL_TIMEOUT_MS = 900_000;

export default defineConfig({
  testDir: resolve(root, "tests/integration"),
  testMatch: "u4-transport-controls-evidence.test.ts",
  ...
  retries: 0,
  workers: 1,
  timeout: U4_TRANSPORT_TEST_TIMEOUT_MS,
  globalTimeout: U4_TRANSPORT_GLOBAL_TIMEOUT_MS,
  expect: {
    timeout: 10_000,
  },
  reporter: [
