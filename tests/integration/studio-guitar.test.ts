...
test("guitar maps the saved Manual notes and refuses stale document/revision/event authority",()=>{
  ...
  const r=c.controller.readGuitar(source);if(!r.ok)throw new Error(r.message);
  expect(r.value.search.midi).toEqual([60,64,67,71]);expect(r.value.externalBass).toBe(false);expect(r.value.search.status).toBe("positions");
  expect(c.readApplicationState()).toBe(before);
  expect(c.controller.readGuitar({...source,documentId:"wrong"}).ok).toBe(false);
  expect(c.controller.readGuitar({...source,eventId:"missing"}).ok).toBe(false);
  expect(c.controller.setTitle("Changed").ok).toBe(true);expect(c.controller.readGuitar(source).ok).toBe(false);
});
test("Auto and Frozen use current inspector realization, never a synthetic keyboard octave",()=>{
  ...
  const auto=c.controller.readGuitar(view.value.source);if(!auto.ok)throw new Error(auto.message);
  expect(auto.value.search.pitches).toEqual(view.value.detail.voicing.activePitches);
  ...
  view=c.controller.readInspector("loop-event-0");if(!view.ok)throw new Error(view.message);
  const frozen=c.controller.readGuitar(view.value.source);if(!frozen.ok)throw new Error(frozen.message);
  expect(frozen.value.search.pitches).toEqual(choice.pitches);expect(frozen.value.search.midi).toEqual(auto.value.search.midi);
});
test("guitar view and existing inspector audition carry the same literal saved notes",async()=>{
  ...
  try{
    ...
    const view=c.controller.readGuitar(source);if(!view.ok)throw new Error(view.message);
    expect(view.value.search.midi).toEqual([60,64,67,71]);
