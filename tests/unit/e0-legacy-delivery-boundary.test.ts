...
import {CANONICAL_JSON_ARTIFACT_SCHEMA,CANONICAL_JSON_MEDIA_TYPE,type CanonicalJsonArtifact,type SemanticDocumentHash,type ExportDeliveryArtifactBinding} from "../../src/export";
...
const malformed:readonly [string,(binding:ExportDeliveryArtifactBinding)=>unknown][]=[
  ...
  ["throwing reflection",b=>new Proxy(cancelled(b),{getPrototypeOf(){throw Error("hostile");}})],
];
...
test("legacy delivery never invokes result getters",async()=>{
...
test("legacy delivery does not retain mutable cleanup receipts",async()=>{
...
test("legacy delivery refuses inherited envelope authority",async()=>{
  const s=await setup(cancelled,raw=>new (class Envelope {completion=Promise.resolve(raw);})());
  expect((await s.complete()).outcome).toBe("delivery-protocol-invalid");expect(s.counts().publications).toBe(0);
});
test("legacy delivery retires rejected completions and permits a fresh preparation",async()=>{
  const s=await setup(cancelled,()=>({completion:Promise.reject(new Error("picker failed"))}));
  const result=await s.complete();expect(result.outcome).toBe("delivery-protocol-invalid");
  if(result.outcome!=="delivery-protocol-invalid")throw Error("Wrong result");
  expect(result.protocolDiagnostic.reason).toBe("threw-or-rejected");expect(result.maximumPossibleOutstandingOwnedResources).toBe(4);
  expect((await s.complete()).outcome).toBe("prepared-export-unavailable");expect((await s.operations.prepareCanonicalExportDelivery({state:s.state})).ok).toBe(true);
});
