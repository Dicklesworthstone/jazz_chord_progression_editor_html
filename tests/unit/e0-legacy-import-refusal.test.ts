...
for (const stage of ["prepare", "retire"] as const) {
  ...
  for (const code of stage === "prepare" ? preparationCodes : retirementCodes) test(`${stage}: preserve literal refusal ${code}`, async () => {
    ...
    expect(result.refusal.code).toBe(code); expect(result.retirementDisposition).toBe("unchanged"); expect(result.state).toBe(request.currentState);
  ...
  const malformed: readonly [string, () => unknown][] = [
    ...
    ["inherited fields", () => Object.create(honest())],
  ...
  for (const field of ["ok", "code"]) test(`${stage}: never read ${field} accessor`, async () => {
