...
for(const theme of ["light","dark"] as const)for(const width of [320,1280])test(`exact QR image to fresh receiver ${theme} ${String(width)}`,async({page,context,browser,browserName},info)=>{
 ...
 try{
  await page.emulateMedia({colorScheme:theme});await page.setViewportSize({width,height:900});await page.goto(url+"#zdoc=2."+Buffer.from(sourceWire).toString("base64url"));await expect(page.locator("#studio-document-title")).toHaveValue(source.title);
  ...
  const receiver=await context.newPage();await receiver.setViewportSize({width,height:900});await receiver.goto(url+link.hash);await expect(receiver.locator("#studio-document-title")).toHaveValue(source.title);const after=await exported(receiver);expect(after).toEqual(before);await receiver.close();
