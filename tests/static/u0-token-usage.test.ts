...
/**
 * The one reviewed responsive re-declaration of a shared token. The
 * coarse-pointer band fix (jcpe-7djg, law U0-ENV-004) grows the transport
 * by overriding its min-block token on :root inside @media (pointer:
 * coarse), so every consumer anchored to the transport height moves with
 * it. The override is conditional, exactly one per reviewed pair, and the
 * base declaration in tokens.css remains the single source of truth — so
 * the duplicate-declaration law admits exactly this (token, file) pair and
 * nothing else.
 */
const RESPONSIVE_TOKEN_OVERRIDES = Object.freeze({
  "--transport-min-block": "src/styles/studio.css",
} as const);
/**
...
describe("TR-U0-TOKENS independently checked production CSS", () => {
  ...
  test("U0-PRIM-012 U0-PRIM-013 U0-VIEW-005 component CSS uses declared semantic tokens without copied color literals", async () => {
    ...
    /*
     * The one admitted duplicate is the reviewed coarse-pointer override
     * (jcpe-7djg, U0-ENV-004): pin its existence, media scope, and value
     * so a mutation that moves, renames, or revalues it fails here — the
     * allowance above is worthless without this counterfactual half.
     */
