...
/**
 * U2/verify (jcpe-milestone-reliable-studio-l3a.11.3) real-browser transport
 ...
 * bundle and driver inputs, and runs the U2 chord-inspector evidence spec
...
export const U2_INSPECTOR_BUN_VERSION = "1.3.14";
export const U2_INSPECTOR_RUNNER_SCHEMA =
  "changes.evidence.u2-inspector-runner.v1";
export const U2_INSPECTOR_INPUT_MANIFEST_SCHEMA =
  "changes.evidence.u2-inspector-input-manifest.v1";
export const U2_INSPECTOR_ENVIRONMENT = Object.freeze({
  runId: "JCPE_U2_EVIDENCE_RUN_ID",
  harnessPath: "JCPE_U2_EVIDENCE_HARNESS_PATH",
  harnessSha256: "JCPE_U2_EVIDENCE_HARNESS_SHA256",
  inputDigest: "JCPE_U2_EVIDENCE_INPUT_DIGEST",
});
...
export type U2InspectorRunPaths = Readonly<{
  runDirectory: string;
...
type InputManifest = Readonly<{
  schema: typeof U2_INSPECTOR_INPUT_MANIFEST_SCHEMA;
...
type RunnerMetadata = Readonly<{
  schema: typeof U2_INSPECTOR_RUNNER_SCHEMA;
  runId: string;
...
function assertRunId(runId: string): void {
  if (!SAFE_RUN_ID.test(runId)) {
    throw new Error(
      "U2_INSPECTOR_RUN_ID_INVALID: run ID must be 8-128 safe filename characters.",
    );
  }
}

export function createU2InspectorRunPaths(
  runId: string,
  root = ROOT,
): U2InspectorRunPaths {
  assertRunId(runId);
...
function repoRelative(path: string): string {
  ...
  if (
    ...
    throw new Error(`U2_INSPECTOR_INPUT_OUTSIDE_ROOT: ${path}`);
  }
  return normalized;
}
...
async function readPlaywrightVersion(): Promise<string> {
  ...
  if (
    ...
    throw new Error("U2_INSPECTOR_PLAYWRIGHT_VERSION_INVALID");
  }
...
async function buildHarness(
  ...
  if (!(await Bun.file(HARNESS_ENTRY).exists())) {
    throw new Error(
      `U2_INSPECTOR_HARNESS_MISSING: ${repoRelative(HARNESS_ENTRY)}`,
    );
  }
  ...
  if (!result.success || result.metafile === undefined) {
    ...
    throw new Error(
      `U2_INSPECTOR_BUNDLE_FAILED${diagnostics.length === 0 ? "" : `: ${diagnostics}`}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      "U2_INSPECTOR_BUNDLE_SHAPE: expected one entry-point script and no side artifacts.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "U2_INSPECTOR_BUNDLE_EXTERNAL_IMPORT: the browser harness must be one self-contained script.",
    );
  }
  ...
  for (const absolutePath of sortedInputPaths) {
    if (!(await Bun.file(absolutePath).exists())) {
      throw new Error(
        `U2_INSPECTOR_INPUT_MISSING: ${repoRelative(absolutePath)}`,
      );
    }
    ...
    if (reportedBytes !== undefined && bytes.byteLength !== reportedBytes) {
      throw new Error(
        `U2_INSPECTOR_INPUT_CHANGED_DURING_BUNDLE: ${repoRelative(absolutePath)}`,
      );
    }
    components.push(
      Object.freeze({
  ...
  if (components.length === 0) {
    throw new Error("U2_INSPECTOR_INPUT_MANIFEST_EMPTY");
  }
  const inputManifest: InputManifest = Object.freeze({
    schema: U2_INSPECTOR_INPUT_MANIFEST_SCHEMA,
    algorithm: "sha256-component-manifest-v1",
...
function metadataFor(
  ...
  return Object.freeze({
    schema: U2_INSPECTOR_RUNNER_SCHEMA,
    runId,
    outcome,
...
function bindPlaywrightEnvironment(
  ...
  process.env[U2_INSPECTOR_ENVIRONMENT.runId] = runId;
  process.env[U2_INSPECTOR_ENVIRONMENT.harnessPath] = prepared.paths.harnessPath;
  process.env[U2_INSPECTOR_ENVIRONMENT.harnessSha256] = prepared.bundle.sha256;
  process.env[U2_INSPECTOR_ENVIRONMENT.inputDigest] =
...
export async function runU2InspectorEvidence(): Promise<{
  metadata: RunnerMetadata;
  exitCode: number;
}> {
  if (Bun.version !== U2_INSPECTOR_BUN_VERSION) {
    throw new Error(
      `U2_INSPECTOR_BUN_VERSION: expected ${U2_INSPECTOR_BUN_VERSION}, received ${Bun.version}.`,
    );
  }
  process.chdir(ROOT);
  const runId = randomUUID();
  const paths = createU2InspectorRunPaths(runId);
...
if (import.meta.main) {
  try {
    const result = await runU2InspectorEvidence();
      ...
      stableJson({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Unknown U2 inspector runner failure.",
      }).trimEnd(),
    );
    process.exitCode = 2;
  }
