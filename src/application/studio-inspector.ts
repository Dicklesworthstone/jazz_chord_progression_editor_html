...
export type StudioInspectorPreview =
...
export function readInspectorSymbolDraft(state: AppState, source: StudioInspectorSource,
  text: string, patch?: StudioInspectorStructurePatch): StudioInspectorResult<StudioInspectorSymbolDraft> {
  ...
  } }, { draftSymbolText: text });
  return Object.freeze({ ok: true, value: Object.freeze({ text, chord: parsed.ok ? parsed.chord : null, detail }) });
}
  ...
  const decoded = decodeDocumentShape({
    ...
    sections: document.sections.map(section => ({ ...section,
      measures: section.measures.map(measure => ({ ...measure,
        events: measure.events.map(current => current.id === event.id ? { ...current, voicing } : current),
      })),
    })),
  });
...
export function readStudioInspector(
  ...
  return Object.freeze({ ok: true, value: Object.freeze({ source, event, detail,
    choices: Object.freeze(choices), unavailableChoices: Object.freeze(unavailableChoices) }) });
}
...
export function prepareInspectorFrozen(
  ...
  const view = readStudioInspector(state, source.eventId, selectedChoice.policy);
  if (!view.ok) return view;
  const fresh = view.value.choices.find(candidate => candidate.id === selectedChoice.id);
