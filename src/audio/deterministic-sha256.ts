...
/** Browser-safe, dependency-free SHA-256 with byte-for-byte known vectors. */
export function sha256Bytes(input: Uint8Array): Uint8Array {
  ...
  for (let index = 0; index < state.length; index += 1) {
    outputView.setUint32(index * 4, state[index] ?? 0);
  }
  return output;
}

export function sha256Hex(text: string): string {
  return Array.from(sha256Bytes(new TextEncoder().encode(text)), (byte) =>
    byte.toString(16).padStart(2, "0"),
  ).join("");
}

export function sha256LowUint32(text: string): number {
  const digest = sha256Bytes(new TextEncoder().encode(text));
  return new DataView(digest.buffer, digest.byteOffset, digest.byteLength).getUint32(0, true);
}
