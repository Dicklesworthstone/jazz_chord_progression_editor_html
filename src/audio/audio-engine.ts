...
function createAudioEngineInternal(
  ...
  function canonicalRenderedChordPairs(
    ...
    if (algorithmId !== undefined && voiced.length > 1) {
      ...
      if (opens !== undefined) {
        ...
        let pitches: number[] = voiced.map((pair) => pair.midiPitch);
        let guard = 0;
        while (
  ...
  async function finishInitialization(
    ...
    if (renderer === null) {
      try {
        renderer = rendererOverridesForTest?.concertGrand ??
          await loadConcertGrandRenderer();
  ...
  function attackAudioVoices(
    ...
    try {
      ...
      if (!validated.ok) return refuse(validated.failure);
      const recipe = recipeForEngine(validated.value.instrumentId);
      /*
  ...
  async function prepareRenderedAudioVoices(
    ...
    const recipe = recipeForEngine(instrumentId.value);
    if (recipe.synthesis !== "rendered") {
      return success(
        Object.freeze({
    ...
    if (
      ...
      for (let index = 0; index < notesValue.length; index += 1) {
        ...
        const explicitEventId = eventIdValue;
        pluckedNotes.push(
          Object.freeze({
